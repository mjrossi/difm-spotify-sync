...
| `internal/syncer/loop.go` (new) | `classify`, `nextDelay`, `maxRetryDelay`; after Task 5, also `minInterval` and `Loop` (pure move) |
...
func TestLoop_RateLimitDelaysTheNextPassByRetryAfter(t *testing.T) {
	...
	// One-shot, not sticky: the pass after a delayed one is back at the
	// interval. Safe to flip the knob here because Loop is parked at its
	// select, so no request is in flight to read it.
...
- [ ] **Step 7: Move `Loop` and `minInterval` into `loop.go`** as a pure, separate commit ("Move Loop and minInterval into loop.go"), so the file that holds the scheduling policy also holds the scheduler.
