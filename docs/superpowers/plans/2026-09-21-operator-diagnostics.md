...
- [ ] **docker/healthcheck.sh** comment: "within DIFMSYNC_STATUS_MAX_AGE (unset: three times DIFMSYNC_INTERVAL)".
- [ ] **CLAUDE.md** health-rule paragraph: `--max-age` unset is 3 × the (floored) interval, decided in `effectiveMaxAge` for both the daemon and `status --check` so the two cannot disagree; the declared default stays 45m for the config-drift test; an empty variable counts as unset (`nonEmptyEnv`). Correct the scan-window sentence: at one row per interval the window is never binding, but a sub-interval `Retry-After` cadence or repeated restarts can stack rows faster and evict the clean row early — that verdict is the better one, so the window stays. Operator surface: `Report` carries `version`, `last_success_at` (the accepted row's time, not a second query), `consecutive_failures`.
